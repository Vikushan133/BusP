...
class _LostScreenState extends State<LostScreen> {
  ...
  Future<void> _loadFoundItems() async {
    try {
      final items = await FirebaseService.getAllFoundItems();
      setState(() {
        _foundItems = items;
        _isLoading = false;
      });
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_selectedImage != null) {
        String fileName = 'lost_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      await FirebaseService.addLostItem(itemData);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Сигналът за изгубен предмет е подаден!'),
  ...
  void _filterItems(String query) {
    if (query.isEmpty) {
      _filteredItems = _foundItems;
    } else {
      _filteredItems = _foundItems.where((item) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_showCreateForm ? 'Подай сигнал за изгубен предмет' : AppStrings.lost),
      ),
        ...
        children: [
          TextFormField(
            ...
            decoration: const InputDecoration(
              labelText: AppStrings.lostObject,
